import pandas as pd
import numpy as np
import os
from sklearn.linear_model import LogisticRegression

# ---------- 1. LOAD GDP ----------
df = pd.read_csv("it_gdp.csv")

df.rename(columns={
    "observation_date": "date",
    "CLVMNACSCAB1GQIT": "gdp"
}, inplace=True)

df["date"] = pd.to_datetime(df["date"])
df = df.sort_values("date")

# ---------- 2. COMPUTE YoY GDP GROWTH ----------
df["gdp_growth"] = df["gdp"].pct_change(4)

# ---------- 3. ROLLING ENTROPY ----------
window = 12

def rolling_entropy(series):
    ent = []
    for i in range(len(series)):
        if i < window:
            ent.append(np.nan)
        else:
            s = series[i-window:i]
            s = s - np.min(s) + 1e-6
            p = s / np.sum(s)
            ent.append(-np.sum(p * np.log(p)))
    return ent

df["entropy"] = rolling_entropy(df["gdp_growth"].values)

# ---------- 4. VOLATILITY ----------
df["volatility"] = df["gdp_growth"].rolling(window).std()

# ---------- 5. CRISIS DUMMY ----------
df["year"] = df["date"].dt.year

crisis_years = [2008, 2011, 2012]
df["crisis"] = df["year"].isin(crisis_years).astype(int)

# ---------- 6. FORWARD 4Q CRISIS ----------
df["crisis_next_4q"] = df["crisis"].shift(-4)

# ---------- 7. CLEAN ----------
df = df.dropna()

# ---------- 8. STANDARDIZE ----------
df["entropy_std"] = (df["entropy"] - df["entropy"].mean()) / df["entropy"].std()
df["volatility_std"] = (df["volatility"] - df["volatility"].mean()) / df["volatility"].std()

# ---------- 9. LOGISTIC REGRESSION ----------
X = df[["entropy_std", "volatility_std"]]
y = df["crisis_next_4q"]

model = LogisticRegression()
model.fit(X, y)

print("ITALY RESULTS")
print("Entropy coefficient:", model.coef_[0][0])
print("Volatility coefficient:", model.coef_[0][1])

# ---------- 10. SAVE RESULTS ----------
os.makedirs("IT_RESULTS", exist_ok=True)
df.to_csv("IT_RESULTS/it_full_dataset.csv", index=False)

print("IT_RESULTS folder fully saved.")