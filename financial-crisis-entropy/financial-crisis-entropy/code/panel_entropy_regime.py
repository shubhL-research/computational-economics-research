...
# =========================================
# ENTROPY REGIME CONSTRUCTION
# =========================================

# 4Q rolling mean (persistence)
panel['entropy_roll4'] = (
    panel.groupby('country')['entropy_std']
    .transform(lambda x: x.rolling(4).mean())
)

# Country-specific mean and std
panel['country_entropy_mean'] = (
    panel.groupby('country')['entropy_std'].transform('mean')
)

panel['country_entropy_std'] = (
    panel.groupby('country')['entropy_std'].transform('std')
)

# Z-score entropy
panel['entropy_z'] = (
    (panel['entropy_std'] - panel['country_entropy_mean'])
    / panel['country_entropy_std']
)

# High entropy regime dummy
panel['high_entropy_regime'] = (
    panel['entropy_z'] > 1
).astype(int)
